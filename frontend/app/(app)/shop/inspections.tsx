...
type Equipment = { id: string; sku: string; qr_code?: string | null; name: string; category: string; pending_inspection: number };
...
export default function InspectionsScreen() {
  ...
  return (
    <Screen title="Inspections" subtitle={`${totalPending} units awaiting inspection`} back
      ...
      <DetailDrawer visible={!!selected} title={selected?.name || "Inspect"} subtitle={selected ? `${equipmentIdentifier(selected)} · ${selected.pending_inspection} awaiting` : undefined} onClose={() => setSelected(null)} testID="inspect-drawer">
        {selected ? (
          <View>
