...
export default function BookingsScreen() {
  ...
  const addItem = (eq: Eq) => {
    setDraft((d: any) => {
      ...
      return { ...d, items: [...d.items, { equipment_id: eq.id, sku: eq.sku, name: eq.name, qty: 1, returned_qty: 0, daily_rate: 0 }] };
    });
  };
  ...
  const capacityColumns = useMemo<ColumnDef<CapacityRow>[]>(() => [
    { key: "sku", label: "SKU", width: 110, render: (row) => <Mono style={styles.tableMono}>{row.sku}</Mono> },
  ...
  return (
    <Screen title="Bookings" subtitle="Pipeline · Capacity" back rightAction={tab === "pipeline" ? { icon: "add", onPress: newBooking, testID: "new-booking-btn" } : undefined}
      ...
      {isShellWide ? (
        ...
        <><Card style={{ marginBottom: spacing.md }}><Input label="Check date (yyyy-mm-dd)" value={capDate} onChangeText={setCapDate} mono autoCapitalize="none" testID="capacity-date" /><Button title="Check Capacity" onPress={loadCapacity} testID="check-capacity-btn" /></Card>{capRows.map((row) => <Card key={row.equipment_id} style={{ marginBottom: spacing.sm }} testID={`cap-row-${row.sku}`}><Row style={{ justifyContent: "space-between" }}><View style={{ flex: 1 }}><Text style={typo.body}>{row.name}</Text><Mono style={{ fontSize: 11, color: colors.inkMuted }}>{row.sku}</Mono></View><Pill color={row.available > 0 ? colors.success : colors.error} bg={row.available > 0 ? colors.successSoft : colors.errorSoft}>{row.available}/{row.quantity}</Pill></Row></Card>)}</>
      )}
        ...
        <Screen title="New Booking" back rightAction={{ icon: "close", onPress: () => setCreating(false), testID: "close-new-booking" }}>
              ...
              <Card key={item.equipment_id} style={{ marginBottom: spacing.sm }} testID={`bk-item-${item.sku}`}>
                <Row style={{ justifyContent: "space-between", alignItems: "center" }}>
                  <View style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <Mono style={{ fontSize: 11, color: colors.inkMuted }}>{item.sku} · {cap ? `${cap.available} available` : "—"}</Mono>
                  </View>
          ...
          <View style={{ borderWidth: 1, borderColor: colors.border, marginBottom: spacing.md }} testID="bk-add-eq-list">
            {equipment
              ...
              .filter((e) => !eqSearch.trim() || `${e.sku} ${e.name}`.toLowerCase().includes(eqSearch.trim().toLowerCase()))
              .slice(0, 8)
              .map((e) => (
                <TouchableOpacity key={e.id} onPress={() => addItem(e)} style={styles.eqRow} testID={`bk-add-eq-${e.sku}`}>
                  <View style={{ flex: 1 }}><Text style={typo.body}>{e.name}</Text><Mono style={{ fontSize: 11, color: colors.inkMuted }}>{e.sku}{availability[e.id] ? ` · ${availability[e.id].available} available` : ""}</Mono></View>
